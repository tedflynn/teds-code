...
WQ <- read_csv("RRI data/spc 1m.csv",
               skip = 4)
...
WQ.3m <- read_csv("RRI data/spc 3m.csv",
                  skip = 4)
#col_types = "dcdddcdcdccccccccc"
#)
...
WQ.3m <- WQ.3m %>% rename("SpCond.3m" = "VALUE")
...
WQ.6m <- read_csv("RRI data/spc 6m.csv",
                  skip = 4)
#col_types = "dcdddcdcdccccccccc"
#)
...
WQ.6m <- WQ.6m %>% rename("SpCond.6m" = "VALUE")
...
WQ.all.long <- WQ.all.long %>% rename("SpCond" = "value")
...
monthly.means <- WQ.all.long %>% 
  group_by(Depth, Month) %>%
  summarize(Mean = mean(SpCond, na.rm = T), SD = sd(SpCond, na.rm = T)) %>%
  ungroup()

mon.mn.plot <- ggplot(monthly.means, aes(x = Month, y = Mean)) +
  ...
  labs(x = "Month",
       ...
       title = "Monthly Mean Sp. Conductance at RRI")

mon.mn.plot + facet_wrap(Depth ~ ., ncol = 1)

ggsave(path="plots",
       filename = "mon.mn.SpCond.plot.png", 
       device = "png",
       scale=1.0, 
       units="in",
       height=6,
...
## Plot Temp at RRI before and after sampling

CDEC.RRI <- cdec_query("RRI", sensors = c("100"),  
                       start.date = as.Date("2021-08-27"), 
                       end.date = as.Date("2021-09-11"))

## Remove midnight-only value on 9/12
CDEC.RRI <- CDEC.RRI %>% filter(DateTime < as.POSIXct("2021-09-12"))

## Plot Daily Temps plus error bars
...
plot.daily.mean.T <- ggplot(daily.means, aes(x = date, y = Mean)) +
  theme(panel.background = element_rect(fill = "white", linetype = 0)) + 
  theme(panel.grid.minor = element_blank()) +
  scale_x_datetime(date_breaks = "2 day", date_labels = "%b %d") +
  geom_point(position=position_dodge(.9), stat="identity", size = 4) +
  geom_hline(aes(yintercept=696.4), color = "blue") +
  #geom_text(y = 23.8, x = as.POSIXct("2021-09-06"), label = "Mean Temperature for September, 2008 - 2020") +
...
ggsave(path="plots",
       filename = "daily.mn.SpCond.Sept.plot.png", 
       device = "png",
       scale=1.0, 
       units="in",
       height=3,
