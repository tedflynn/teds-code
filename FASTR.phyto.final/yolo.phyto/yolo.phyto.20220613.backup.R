...
setwd("C:/Users/tflynn.WATER/Documents/R/FASTR.phyto.redo/yolo.phyto")
...
sort(unique(phyto$Genus)) ## 152 unique genera
...
phyto.BV.grp <- phyto.BV.grp %>% rename("BV.Density" = "Conc")
phyto.BV.gen <- phyto.BV.gen %>% rename("BV.Density" = "Conc")
...
temp <- temp %>%
  mutate(Diatoms = 0.288*(Diatoms^0.811)/1e3, .keep = "unused") %>%
  mutate(Cyanobacteria = 0.216*(Cyanobacteria^0.939)/1e3, .keep = "unused") %>%
  mutate(`Green Algae` = 0.216*(`Green Algae`^0.939)/1e3, .keep = "unused") %>%
  mutate(`Golden Algae` = 0.216*(`Golden Algae`^0.939)/1e3, .keep = "unused") %>%
  mutate(Cryptophytes = 0.216*(Cryptophytes^0.939)/1e3, .keep = "unused") %>%
  mutate(Dinoflagellates = 0.216*(Dinoflagellates^0.939)/1e3, .keep = "unused") %>%
  mutate(Ciliates = 0.216*(Ciliates^0.939)/1e3, .keep = "unused") %>%
  mutate(Other = 0.216*(Other^0.939)/1e3, .keep = "unused")
...
temp <- pivot_wider(phyto.BM.grp, names_from = Group, values_from = Biomass, values_fill = 0)
...
phyto.LCEFA.grp <- pivot_longer(temp, cols = 9:last_col(),
                                  names_to = "Group", 
                                  values_to = "LCEFA")
...
temp <- pivot_longer(phyto.grp, cols = 10:last_col(), 
                     names_to = "DataType", 
                     values_to = "Amount")
